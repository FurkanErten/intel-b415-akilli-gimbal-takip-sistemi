"""
TR: Intel B415/RealSense kamera, YOLO tabanlı yüz algılama, HSV tabanlı kırmızı/mavi hedef algılama,
derinlik ölçümü ve iki eksenli NEMA17 gimbal kontrolü için ana uygulama dosyası.

EN: Main application for Intel B415/RealSense camera, YOLO-based face detection, HSV-based red/blue
object detection, depth estimation and two-axis NEMA17 gimbal control.

Donanım / Hardware:
    - 2 adet NEMA17 step motor
    - 2 adet TB6600 step motor sürücüsü
    - 1 adet Arduino Uno
    - 1 adet Intel B415/RealSense RGB-D kamera

Arduino seri haberleşme protokolü / Serial protocol:
    V <pan_hizi> <tilt_hizi>

Çalışma tuşları / Runtime keys:
    Q : çıkış / quit
    S : motorları durdur / stop motors
    F : yüz algılamayı aç/kapat / face detection on/off
    K : kırmızı hedef algılamayı aç/kapat / red target detection on/off
    M : mavi hedef algılamayı aç/kapat / blue target detection on/off
    N veya D : gece görüş modunu aç/kapat / night enhancement on/off
    A : otomatik gece görüş modu / automatic enhancement mode
    R : ham görüntü modu / raw image mode
    E : HAM -> OTO -> SUREKLI mod geçişi / mode cycle
    B : Gamma / DeepFusion arka ucunu değiştir / switch backend
    T : hedef seçimi politikasını değiştir / switch target policy
...
def direction_text(sign: float) -> str:
    return "DUZ" if sign >= 0 else "TERS"
...
class LowLightEnhancer:
    ...
    1) deepfusion: tries to load modeller/dusuk_isik_modeli.pth + dusuk_isik_modeli.py
    ...
    def __init__(
        ...
        model_path: str = "modeller/dusuk_isik_modeli.pth",
        module_path: str = "dusuk_isik_modeli.py",
        class_name: str = "",
        input_size: int = 320,
        fp16: bool = True,
    ...
    def _try_load_deepfusion(self) -> bool:
        try:
            ...
            if sd is None:
                print("[WARN] modeller/dusuk_isik_modeli.pth içinde state_dict bulunamadı.")
                return False
            ...
            if best_model is None:
                print("[WARN] dusuk_isik_modeli.py içinden no-arg nn.Module instantiate edilemedi.")
                return False
...
class FaceRecognizer:
    def __init__(self, db_path: str, threshold: float = 0.40):
        ...
        if not db_path or not os.path.exists(db_path):
            print(f"[UYARI] Yüz veritabanı bulunamadı: {db_path}. Kimlik tanıma kapalı.")
            return
        ...
        self.enabled = True
        print(f"[OK] Yüz kimliği aktif. Kişi sayısı: {len(self.names)} | Sağlayıcı: {self.provider_text}")
...
class RuntimeModes:
    ...
    def summary(self) -> str:
        return (
            f"YUZ:{'ACIK' if self.face else 'KAPALI'} | "
            f"KIRMIZI:{'ACIK' if self.red else 'KAPALI'} | "
            f"MAVI:{'ACIK' if self.blue else 'KAPALI'} | "
            f"HEDEF:{self.target_policy.upper()}"
        )
...
def detection_draw_color(d: Dict[str, Any], is_target: bool) -> Tuple[int, int, int]:
    ...
    if d.get("source") == "blue":
        return (255, 0, 0)
    if d.get("name") == "BILINMIYOR":
        return (0, 0, 255)
    return (0, 255, 0)


def detection_label(d: Dict[str, Any], recognizer: Optional[FaceRecognizer]) -> str:
    dist_text = "derinlik yok" if d.get("distance") is None else f"{d['distance']:.2f}m"
    src = d.get("source", "face")
    if src == "face":
        if recognizer is not None and recognizer.enabled:
            return f"YUZ | {d['name']} {d['id_score']:.2f} | {dist_text}"
        return f"YUZ {d['conf']:.2f} | {dist_text}"
    if src == "red":
...
def parse_args():
    ...
    p.add_argument("--yolo", type=str, default="modeller/yuz_tespit_yolov11l.pt", help="YOLO yüz modeli dosya yolu. Varsayılan: modeller/yuz_tespit_yolov11l.pt")
    ...
    # Çalışma zamanı algılama modülleri. Stabil FPS için varsayılan: yüz açık, renkler kapalı.
    ...
    p.add_argument("--db", type=str, default="yuz_veritabani.npz", help="Yüz kimliği veritabanı dosya yolu")
    p.add_argument("--no-id", action="store_true", help="Kimlik tanımayı kapat")
    p.add_argument("--threshold", type=float, default=0.40, help="Yüz kimliği eşik değeri")
    p.add_argument("--track-name", type=str, default="", help="Sadece bu kişiyi takip et")
    p.add_argument("--known-only", action="store_true", help="BILINMIYOR hedefleri takip etme")
    ...
    p.add_argument("--depth-width", type=int, default=640, help="Derinlik genişliği. 1080p RGB'de bile 640 önerilir.")
    p.add_argument("--depth-height", type=int, default=480, help="Derinlik yüksekliği. 1080p RGB'de bile 480 önerilir.")
    p.add_argument("--depth-fps", type=int, default=30, help="Derinlik FPS")
    ...
    # HSV renk algılama ayarları. Dış ortam gürültüsü için S/V eşikleri bilinçli olarak temkinli seçilmiştir.
    ...
    p.add_argument("--dark-model", type=str, default="modeller/dusuk_isik_modeli.pth", help="Düşük ışık modeli dosya yolu")
    p.add_argument("--dark-module", type=str, default="dusuk_isik_modeli.py", help="Düşük ışık model mimarisi dosya yolu")
    p.add_argument("--dark-class", type=str, default="", help="Biliniyorsa model mimarisi sınıf adı")
    ...
    p.add_argument("--dark-fp16", dest="dark_fp16", action="store_true", default=True, help="CUDA üzerinde düşük ışık modeli FP16. Varsayılan aktif.")
    p.add_argument("--no-dark-fp16", dest="dark_fp16", action="store_false", help="Düşük ışık FP16 modunu kapat")
    p.add_argument("--trust-model-code", action="store_true", help="Tam PyTorch modelini ve dusuk_isik_modeli.py dosyasını güvenerek yüklemek için")
...
def select_target(detections: List[Dict[str, Any]], args, modes: Optional[RuntimeModes] = None) -> Optional[Dict[str, Any]]:
    ...
    if args.known_only:
        candidates = [d for d in candidates if d["name"] not in ["BILINMIYOR", "YUZ"]]
    if not candidates:
        return None
...
def main():
    ...
    print(f"[INFO] RealSense derinlik: {args.depth_width}x{args.depth_height}@{args.depth_fps}")
    profile = pipeline.start(config)

    depth_sensor = profile.get_device().first_depth_sensor()
    depth_scale = depth_sensor.get_depth_scale()
    print(f"[INFO] Derinlik ölçeği: {depth_scale}")
    ...
    try:
        while True:
            ...
            # Yüz modülü: yüz kapalıyken maliyetli YOLO çıkarımı atlanır.
            if modes.face:
                result = yolo.predict(
                    source=infer_img,
                    imgsz=args.imgsz,
                ...
                if result.boxes is not None:
                    for b in result.boxes:
                        ...
                        detections.append({
                            "box": box,
                            "conf": conf,
                            "class_name": "YUZ" if "face" in str(class_name).lower() else class_name,
                            "name": name,
                            "id_score": id_score,
                            "distance": dist,
                            "xyz": xyz,
            ...
            motor_status = "ACIK" if gimbal.enabled else "KAPALI"
            dir_status = f"P{signed_text(controller.pan_sign)} T{signed_text(controller.tilt_sign)}"
            enh_status = f"{enhancer.mode.upper()}:{enh_backend.upper() if enhanced else 'HAM'}[{enhancer.backend.upper()}]"
            cv2.putText(
                view,
                f"FPS:{fps:.1f} | {device} | FP16:{int(yolo_half)} | GORUNTU:{enh_status} parlaklik:{luma_score:.1f} | Motor:{motor_status} | YON:{dir_status}",
            ...
            cv2.putText(
                view,
                "Tuslar: F yuz | K kirmizi | M mavi | N/D gece | A oto | R ham | B arka uc | T hedef | S dur | Q cikis",
            ...
            if target is not None:
                ...
                if smooth_target_dist is not None:
                    cv2.putText(
                        view,
                        f"HEDEF: {target['class_name']} | MESAFE: {smooth_target_dist:.2f} m",
                        (10, 88), cv2.FONT_HERSHEY_SIMPLEX, 0.70, (0, 255, 255), 2,
                    )
                else:
                    cv2.putText(
                        view,
                        f"HEDEF: {target['class_name']}",
                        (10, 88), cv2.FONT_HERSHEY_SIMPLEX, 0.70, (0, 255, 255), 2,
                    )
            ...
            cv2.imshow("RealSense + Yuz/Kirmizi/Mavi + Gece Gorus + Derinlik + Gimbal", show_view)
