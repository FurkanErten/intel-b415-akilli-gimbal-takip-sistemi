# TR: Bu dosya eski sürüm arşivi olarak tutulmuştur. Ana çalışma dosyası: ana_takip_sistemi.py
# EN: This file is kept as an archived legacy version. Main file: ana_takip_sistemi.py
...
- Low-light enhancement can use your modeller/dusuk_isik_modeli.pth + dusuk_isik_modeli.py if available.
- If the trained model cannot be loaded, it falls back to a fast GPU gamma enhancer.

V7 fast dark fixes:
- Default low-light backend is now fast GAMMA instead of heavy DeepFusion.
- B key toggles GAMMA / DEEPFUSION without restarting.
...
def parse_args():
    ...
    p.add_argument("--yolo", type=str, default="modeller/yuz_tespit_yolov11n.pt", help="YOLO model path. Face, person, object fark etmez.")
    ...
    p.add_argument("--db", type=str, default="yuz_veritabani.npz", help="Face ID DB path")
    ...
    p.add_argument("--enhance-backend", choices=["auto", "deepfusion", "gamma"], default="gamma", help="FAST için gamma varsayılan. Kalite için D/B ile veya --enhance-backend deepfusion kullan.")
    p.add_argument("--dark-th", type=float, default=70.0)
    p.add_argument("--dark-model", type=str, default="modeller/dusuk_isik_modeli.pth", help="Dark project modeller/dusuk_isik_modeli.pth path")
    p.add_argument("--dark-module", type=str, default="dusuk_isik_modeli.py", help="Dark project dusuk_isik_modeli.py path")
    ...
    p.add_argument("--trust-model-code", action="store_true", help="Full PyTorch model yüklemek ve dusuk_isik_modeli.py import etmek için")
...
def main():
    ...
    print("[INFO] Başladı. Q çıkış | S stop | D dark aç/kapat | A auto | R raw | E mod | B gamma/deepfusion | 1/2/3 motor yön")
    ...
    try:
        while True:
            ...
            cv2.putText(
                view,
                "Keys: D dark | A auto | R raw | E mode | B backend | S stop | 1/2/3 dir | Q",
            ...
            cv2.imshow("RealSense + Dark Enhance + YOLO Depth + Gimbal", show_view)
